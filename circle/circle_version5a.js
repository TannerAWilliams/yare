...
function energize_and_merge_spirit(spirit1) {
    if(spirit1.sight.friends.length > 0) {
        for(friendly_id of spirit1.sight.friends) {
            ...
            } if(distance <= 100 && spirit1.mark == 'energize home base' && spirit1.energy != spirit1.energy_capacity){
...
function take_spirit_action(spirit1) {
    if (spirit1.mark == "defense") {
	   ...
	   harvest_stars_queue(spirit1)
    ...
    } else if (spirit1.mark == "attack enemy star") {
        enemy_star = star_zxq
	    ...
	    if(spirit1.sight.enemies.length == 0) {
	        ...
	        spirit1.move(enemy_star.position)
            spirit1.energize(spirit1)
	    }
	}
}

function energize_home_base(spirit1) {
    // outpout
    if(unit_distance(spirit1, outpost) <= unit_distance(spirit1, base)){
        if(outpost.energy < 1000){
            spirit1.move(outpost.position)
            spirit1.energize(outpost)
        }
        else{
...
function set_engagament_protocol(spirit1){
...
function initiate_offensive_strategy(spirit1) {
    if(memory['spirits_energy'] >= 700) {
        spirit1.set_mark("attack enemy star")
        return true
    }
    return false
}
...
function harvest_stars_queue(spirit1) {
    stars_by_distance = stars_ordered_by_distance(spirit1)

    // go to middle star
    if(stars_by_distance[0] != star_p89 && memory['spirits'] >= 20 && spirit_id(spirit1) % 5 == 0){
        spirit1.move(star_p89.position)
        spirit1.energize(star_p89)
    } else {
        spirit1.move(stars_by_distance[0].position)
        spirit1.energize(spirit1)
    }
...
function attack_closest_enemy(spirit1) {
    if(spirit1.sight.enemies.length > 0) {
        ...
        distance_from_enemy = unit_distance(spirit1, closest_enemy)

	    spirit1.move(closest_enemy.position)
	    spirit1.energize(closest_enemy)
    }
    else if(unit_distance(spirit1, enemy_base) < 750) {
        spirit1.move(enemy_base.position)
        spirit1.energize(enemy_base)
    }
    else {
